...
var _TweenMax = __webpack_require__(16);

var _scrollOut = _interopRequireDefault(__webpack_require__(18));

var _signupInfo = _interopRequireDefault(__webpack_require__(19));
...
function () {
  ...
  _createClass(Home, [{
    ...
    key: "BannerGallerySlide",
    value: function BannerGallerySlide() {
    ...
    key: "BannerImageSwapper",
    value: function BannerImageSwapper() {
      var _this = this;

      this.currentBannerGallery = "xmen";
      this.$slideSwapBtn = $("#yadea-xmen-news-banner .slide-swap");
      this.$slideSwapBtn.on("click", function (e) {
        var $clickedBtn = $(e.target);

        if (!$clickedBtn.hasClass("active")) {
          var targetValue = $clickedBtn.data("target"); // Button Process

          _this.$slideSwapBtn.removeClass("active");

          $clickedBtn.addClass("active"); // Slider Process

          $("#".concat(_this.currentBannerGallery, "-gallery-slider")).parents(".main-banner-slider").removeClass("active");
          $("#".concat(targetValue, "-gallery-slider")).parents(".main-banner-slider").addClass("active"); // Assign new current version

          _this.currentBannerGallery = targetValue;
        }
      });
    }
  }, {
    key: "SetupImageViewer",
    value: function SetupImageViewer() {
      var _this2 = this;

      this.currentImage = "";
      this.$viewerImages = $("#yadea-xmen-news-banner .image-viewer");
      this.$viewerImageFrame = $("#banner-image-view-holder");
      this.$closeImageViewer = $(".close-image-viewer"); // Open Image Viewer
      ...
      $(document).on("click", ".thumbnail-holder", function (e) {
        ...
        if ($(e.target).parents(".thumbnail-holder").length > 0) {
          ...
          var imageUrl = $clickedThumbnail.data("image"); // New Image Need To Load

          if (_this2.currentImage !== imageUrl) {
            var imageToShow = new Image();
            imageToShow.src = imageUrl;

            _this2.$viewerImageFrame.html(imageToShow);

            imageToShow.onload = function () {
              _this2.$viewerImages.addClass("active");

              _this2.currentImage = imageUrl;
            };
          } else {
            // Same Image
            _this2.$viewerImages.addClass("active");
        ...
        _this2.$closeImageViewer.on("click", function () {
          _this2.$viewerImages.removeClass("active");
        });
      });
    }
  }, {
    key: "SwitchNews",
    value: function SwitchNews() {
      var target = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "type-sum";
      var $HightlightBlock = $(".feature-posts-layout");
      var $PostItems = $(".post-listing-layout .post-item");
      $PostItems.hide();
      $(".post-listing-layout .post-item.".concat(target)).show();
    ...
    key: "NewsSwitcherSetup",
    value: function NewsSwitcherSetup() {
      var _this3 = this;

      this.$NewsSwitchBtn = $(".news-menu-bar .category-item");
      this.$NewsSwitchBtn.on("click", function (e) {
        var $targetButton = $(e.target);

        if (!$targetButton.hasClass("active")) {
          // Target News Group To Switch To
          var targetData = $targetButton.data("news");
          console.log(targetData); // Button Stage Toggling

          _this3.$NewsSwitchBtn.removeClass("active");

          $targetButton.addClass("active"); // Switch News Type

          _this3.SwitchNews(targetData);
        }
      });
    }
  }]);
